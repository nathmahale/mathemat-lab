...
calcRouter.use(function(request, response, next){
    ...
    if ( !firstStr ) {
        return error(response, {status : 400, message : "Missing required parameter '1st'" });
    }
    if ( !secondStr ) {
        return error(response, {status : 400, message : "Missing required parameter '2nd'" });
    }
    var first = parseFloat(firstStr);
    var second = parseFloat(secondStr);
    if ( isNaN(first) ) {
        return error(response, {status : 400, message : "The parameter '1st' is not a number" });
    }
    if ( isNaN(second) ) {
        return error(response, {status : 400, message : "The parameter '2nd' is not a number" });
    }
