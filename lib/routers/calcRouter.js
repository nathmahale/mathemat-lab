...
calcRouter.use(function (request, response, next) {
  const firstStr = request.query.first
  const secondStr = request.query.second

  if (!firstStr) {
    return error(response, {
      status: 400,
      message: "Missing required param 'first'"
    })
  }
...
calcRouter.route('/sqrt').get(function (request, response) {
  return validResponse(response, Math.sqrt(request.first))
})
