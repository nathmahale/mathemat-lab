...
calcRouter.use(function (request, response, next) {
    ...
    if (!firstStr) {
        return error(response, { status: 400, message: "Missing required param 'first'" });
    }
    if (!secondStr) {
...
calcRouter.route('/expo')
    .get(function (request, response) {
        return validResponse(response, Math.pow(request.first, request.second));
    });

calcRouter.route('/sine')
    .get(function (request, response) {
        return validResponse(response, Math.sin(request.first));
    });
calcRouter.route('/cosine')
    .get(function (request, response) {
        return validResponse(response, Math.cos(request.first));
    });
calcRouter.route('/tan')
    .get(function (request, response) {
        return validResponse(response, Math.tan(request.first));
    });
calcRouter.route('/acos')
    .get(function (request, response) {
        return validResponse(response, Math.acos(request.first / request.second));
    });
// error handler
